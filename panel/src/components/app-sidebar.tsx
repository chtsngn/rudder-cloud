  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* 1. YELKEN DOLGUSU (Stroke YOK -> En soldaki çizgi tamamen silindi) */}
        <path
          d={
            collapsed
      ...
      <div className="relative z-10 flex flex-1 flex-col h-full overflow-hidden">
        ...
        {/* ── 1. HEADER: LOGO + RUDDER + ELİT AÇMA/KAPAMA BUTONU ── */}
        <div
          className={cn(
            "flex items-center justify-between shrink-0 transition-all duration-300",
            collapsed ? "h-20 flex-col justify-center gap-1.5 px-0 pt-2" : "h-24 px-5 pr-8"
          )}
        >
          ...
          {/* Elit Altın Dümen / Panel Açma-Kapatma Butonu */}
          <button
            type="button"
            onClick={toggle}
            title={collapsed ? "Menüyü Genişlet" : "Menüyü Daralt"}
            className={cn(
              "flex items-center justify-center rounded-lg p-1.5 transition-all cursor-pointer",
              "bg-[#dfc9a0]/15 hover:bg-[#dfc9a0]/30 border border-[#dfc9a0]/40 text-[#dfc9a0] hover:scale-105 active:scale-95 shadow-sm",
              collapsed ? "size-6 mt-1" : "size-8"
            )}
          >
            {collapsed ? (
              <PanelLeftOpen className="size-4 text-[#dfc9a0]" />
            ) : (
              <PanelLeftClose className="size-4 text-[#dfc9a0]" />
            )}
          </button>
        </div>
        ...
        {/* İnce Ayırıcı Şerit */}
        <div
          className={cn("h-px transition-all duration-300", collapsed ? "mx-3" : "mx-5 mr-10")}
          style={{
          ...
          {navItems.map((item) => {
            ...
            return (
              <Link
                ...
                className={cn(
                  ...
                  collapsed
                    ...
                    : "rounded-l-xl rounded-r-lg px-4 py-3",
                  active
              ...
              >
                ...
                {/* ═══ SARI ŞERİTLE BİRLEŞİK İÇE DOĞRU BAKAN ALTIN ÜÇGEN OK ═══ */}
                {active && !collapsed && (
